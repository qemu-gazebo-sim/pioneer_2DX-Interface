...
void task_bluepill_code(void *pvParameters){
    ...
    while (true) {
        ...
        if (
            current_connected_state != bluepill_comm->is_bluepill_connected()
            ...
            if(need_send_connect_msg && msg_queue_conn_command != 0) {
                if (xQueueSend(
                        msg_queue_conn_command, 
                        &current_connected_state, 
                        portMAX_DELAY
                    ) == pdTRUE
                ) {
